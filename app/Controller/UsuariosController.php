...
class UsuariosController extends AppController
{
    ...
    public function liquidacion($id){
        ...
        $viajes = $this->Usuario->getLiquidacion($id);
        //los paso a la vista
        $this->set(compact('empresa', 'usuario' , 'viajes' ));
   ...
   function balance($id = null)
    {
        ...
        $this->Viaje = ClassRegistry::init('Viaje');
        $pagos = $this->Viaje->getBalanceUsuario($usuario['Usuario']['id']);
        $this->set(compact('usuario','pagos'));
    }
...
//    }
    ...
    function cliente($id = null)
    {
        ...
        $ivrClientes = $this->Usuario->getClientes();
        //paso las empresas a la vista
        $this->set(compact('empresas', 'ivrClientes'));
    }

    function relacionar($user,$cli){

        $res = $this->Usuario->setRel($user,$cli);

        if ($res)
            $this->Session->setFlash(__('Se ha relacionado el cliente correctamente'), 'success');
        else
            $this->Session->setFlash(__('Error al relacionar cliente'), 'error');
        
        $this->redirect('/usuarios/view');
    }
