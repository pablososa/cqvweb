...
function add() {
        ...
        if ($this->request->data) {
            ...
            if ($this->MapaTermico->save($this->request->data['Evento'])) {
                    ...
                    $this->Session->setFlash(__('El viaje ha sido guardado exitosamente.'), 'success');
                    $this->redirect(
                            array(
    ...
    function edit($id = null) {
        ...
        if ($this->request->data) {
            ...
            if ($result) {
                    $this->MapaTermico->commit();
                    //informo por pantalla
                    $this->Session->setFlash(__('Guardado con exito.'), 'success');
                    $this->redirect(
                            array(
                                'controller' => 'mapaTermicos',
                                'action' => 'index'
    ...
    $this->MapaTermico->id = $id;
        if (!$this->MapaTermico->exists()) {
            throw new NotFoundException(__('Viaje inválido'));
        }
        $result = $this->MapaTermico->begin();
        $result &= $this->MapaTermico->delete();
        if ($result) {
            $this->MapaTermico->commit();
            $this->Session->setFlash(__('El viaje ha sido eliminado con exito'), 'success');
        } else {
            $this->MapaTermico->rollback();
            $this->Session->setFlash(__('El viaje no pudo ser borrado. Por favor, intente nuevamente.'), 'error');
        }
