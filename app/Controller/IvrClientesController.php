...
class IvrClientesController extends AppController {
    ...
    public function index() {
        ...
        $this->Paginator->settings['IvrCliente']['order']['IvrCliente.nombre'] = 'asc';
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            //$this->IvrCliente->crear_usuario( $this->request->data['IvrCliente'] );
            $this->request->data['IvrCliente']['empresa_id'] = $empresa['Empresa']['id'];
            if ($this->IvrCliente->save($this->request->data)) {
                $this->Session->setFlash(__('The ivr cliente has been saved.'), 'success');
    ...
    public function liquidacion($id){
        ...
        $cliente = $this->IvrCliente->findById($id);

        $viajes = $this->IvrCliente->getLiquidacion($id);
        //los paso a la vista
        $this->set(compact('empresa', 'cliente','viajes' ));
    ...
    function liquidar($cliente_id,$usuario_id,$monto) {
         $empresa = $this->Session->read('Empresa');
        ...
        $this->IvrCliente->id = $cliente_id;

        $res = $this->IvrCliente->setLiquidacion($cliente_id,$usuario_id,$monto);

        if ($res)
            $this->Session->setFlash(__('Se ha liquidado correctamente el período correspondiente'), 'success');
        else
            $this->Session->setFlash(__('Error al ingresar la liquidacion correspondiente'), 'error');
        
        $this->redirect('/ivr_clientes/liquidacion/'.$cliente_id);
