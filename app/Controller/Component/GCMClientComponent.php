...
class GCMClientComponent extends Component
{
    ...
    public function initialize(Controller $controller)
    {
		...
		$empresa = $controller->Session->read('Empresa');
		
		$this->fcm_key = $empresa['Empresa']['fcm_key'];
        $this->Gcm = new Gcm();
    }
    ...
    private function sendNotification($type, $id, $title, $data)
    {
        ...
        if (!empty($gcm['Gcm']['id_gcm'])) {
            $result = GCMClient::sendNotification($gcm['Gcm']['id_gcm'], $title, $data,$this->fcm_key);
    ...
    private function sendNotificationAllUsers($title,$empresa_id)
    {
        $options = array(
            'conditions' => array(
        ...
        $usuarios = $this->Gcm->find('all', $options);
        $data = array();
        foreach ($usuarios as $usuario) {

             $this->sendNotification('user', $usuario['Gcm']['usuario_id'],$title,$data);
        }
...
//        }
    ...
    public function sendNotificationVehicle_CerrarSession($vehicle_id = null)
    {
        $data = array(
            'IdVehiculo' => $viaje['Viaje']['id']
        );
        return $this->sendNotificationVehicle($vehicle_id, 'CerrarSession', $data);
    }
