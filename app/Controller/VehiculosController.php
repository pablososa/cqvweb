...
//                $this->sendActivationMail($this->request->data['Vehiculo'], $empresa['Empresa']);
    ...
    function liquidar($id = null,$fecha,$monto) {
        ...
        $usr = $_SESSION['Empresa']['Operador']['usuario'];
        $res = $this->Vehiculo->setLiquidacion($id,$fecha,$monto,$usr);
    ...
    function ingmov($fecha,$det,$monto) {
         $empresa = $this->Session->read('Empresa');
        //pregunto si hay una empresa logueada
        if (!$empresa) {
            //si no hay una empresa logueada, informo
        ...
        $usr = $_SESSION['Empresa']['Operador']['usuario'];
        $res = $this->Vehiculo->setMov($fecha,$det,$monto,$usr);

        if ($res)
            $this->Session->setFlash(__('Se ha registrado correctamente el movimiento '), 'success');
        else
            $this->Session->setFlash(__('Error al ingresar el movimiento'), 'error');
        
        $this->redirect('/vehiculos/balance');

    }



    function liqrango($id = null,$fechaini,$fechafin,$monto) {
         $empresa = $this->Session->read('Empresa');
        //pregunto si hay una empresa logueada
        if (!$empresa) {
            //si no hay una empresa logueada, informo
        ...
        $fechatxt = date("d-m-Y", strtotime($fecha) );
        $usr = $_SESSION['Empresa']['Operador']['usuario'];
        $res = $this->Vehiculo->setLiqRango($id,$fechaini,$fechafin,$monto,$usr);

        if ($res)
            $this->Session->setFlash(__('Se ha liquidado correctamente el período '), 'success');
        else
            $this->Session->setFlash(__('Error al ingresar la liquidacion '), 'error');
        
        $this->redirect('/vehiculos/dash');
    ...
    public function liquidacion($id){
        ...
        $this->set('empresa', $empresa);
         ...
         if ($this->request->data) {
            ...
            $hini = $this->request->data['Vehiculo']['hora_ini']; 
            $hfin = $this->request->data['Vehiculo']['hora_fin'];

            $vid = $this->request->data['Vehiculo']['vid'];

            $viajes = $this->Vehiculo->getFacturacion($fini,$ffin,$hini,$hfin);
            $vehiculos = $this->Vehiculo->getDetalle($fini,$ffin,$hini,$hfin);
            
         }else {
            $viajes = $this->Vehiculo->getInicio();
            $vehiculos = $this->Vehiculo->getDetalleIni();
            }
        //los paso a la vista
        $this->set(compact('empresa', 'viajes', 'fini', 'ffin','hini','hfin','vehiculos','vid' ));
    ...
    public function balance(){
        ...
        $this->set('empresa', $empresa);
         ...
         if ($this->request->data) {
            ...
            $vehiculos = $this->Vehiculo->getMovimientos($fini,$ffin);
            
         }else {
            $vehiculos = $this->Vehiculo->getMovimientosIni();
            }

        
        //los paso a la vista
        $this->set(compact('empresa', 'viajes', 'fini', 'ffin','vehiculos'));
