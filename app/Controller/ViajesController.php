...
class ViajesController extends AppController
{
    ...
    private function _add($empresa, $data, $telefono = null)
    {
        ...
        if (isset($data['Viaje']['des']))
        {

            $this->Viaje->id = $data['Viaje']['des'];
            $this->Viaje->saveField('estado', 'Despachado');
        }

        /*
        if ($empresa['Empresa']['despacho_auto']==0){
            $data['Viaje']['estado'] = "Despacho_pendiente";
        }
        */
        ...
        if ($data['Viaje']['des']!="") $tel = $data['Viaje']['des'];
        
        if ($tel!=""){
            if ($data['Viaje']['des2']!="") $tel = $data['Viaje']['des2'];
        }

        if ($tel!="") {
            
            App::import('Model', 'IvrLlamadaEntrante'); // mention at top
            $llament = new IvrLlamadaEntrante;
            $llament->deleteByEmpresaAndTelefono($empresa['Empresa']['id'],$tel );
        ...
        if (!empty($this->request->data)) {
            ...
            if(empty($viaje)) {
                ...
                if (!empty($viaje)) {
                    ...
                    $this->redirect(['controller' => 'viajes', 'action' => 'adminHistory']);
                } else {
    ...
    public function adminAdd($vehiculo_id = null, $lat = null, $lng = null, $telefono = null,$dir=null,$obs=null,$des=null,$des2=null)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            $result = $this->_add($empresa, $this->request->data, $des2);
            if ($result) {
                $this->Session->setFlash($this->Viaje->message, 'success');
            } else {
                $this->Session->setFlash($this->Viaje->message, 'error');
        ...
        if (!empty($lat) && !empty($lng)) {
            ...
            if ($dir!="" and $dir!=null)
            $this->request->data['Viaje']['dir_origen'] = $dir;
        }
        ...
        $this->request->data['Viaje']['observaciones'] = $obs;
        $this->request->data['Viaje']['des2'] = $des2;


        $this->set(compact('conductors', 'empresa', 'vehiculo_id','des','des2'));
    }
    ...
    public function reasignar2($viaje_id)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if (count($cercanos)==2 and $cercanos[1]=="")
            {
                return;
                //$cercanos[] = $cercanos[0];
