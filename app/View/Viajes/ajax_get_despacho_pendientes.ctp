            ...
            <tr>
                <td>
                    ...
                    if(!empty($despacho_pendiente['IvrDomicilio']['IvrCliente']['telefono'])) {
                        echo $this->Html->link($despacho_pendiente['IvrDomicilio']['IvrCliente']['telefono'].' CC Nº '.str_pad($despacho_pendiente['IvrDomicilio']['IvrCliente']['id'], 10, "0", STR_PAD_LEFT), array('controller' => 'ivr_clientes', 'action' => 'view', $despacho_pendiente['IvrDomicilio']['IvrCliente']['telefono']));
                ...
                <td class="actions">
                    ...
                    <?php 
                    //echo $this->Html->link(__('Crear Viaje'), array('controller' => 'viajes', 'action' => 'createFromDespachoPendiente', $despacho_pendiente['Viaje']['id']), array('class' => 'js-create-viaje')); 
                    //echo $this->Html->link(__('Crear Viaje'), array('controller' => 'viajes', 'action' => 'reasignar', $despacho_pendiente['Viaje']['id']), array('class' => 'js-create-viaje'));
                    echo '<a href="javascript:crearv('.$despacho_pendiente['Viaje']['latitud_origen'].','.$despacho_pendiente['Viaje']['longitud_origen'].','."'".$despacho_pendiente['IvrDomicilio']['domicilio']."'".','."'".(empty($despacho_pendiente['IvrDomicilio']['IvrCliente']['telefono']) ? '' : 'Tel ' . $despacho_pendiente['IvrDomicilio']['IvrCliente']['telefono']." ").$despacho_pendiente['Viaje']['observaciones']."'".','.$despacho_pendiente['Viaje']['id'].','."'".$despacho_pendiente['IvrDomicilio']['IvrCliente']['telefono']."'".')">Crear Viaje</a>';
                    ?>
...
function crearv(lat,lon,dir,obs,des,tel){
    ...
    obs = obs.replace("/", "-");
    window.apptaxiweb.popup('/viajes/adminAdd/0/' + lat + '/' + lon + '/0/'+encodeURI(dir)+'/'+ encodeURI(obs), options);
