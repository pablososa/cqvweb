...
class Viaje extends AppModel
{
    ...
    public function add($empresa, $data)
    {
        ...
        if (empty($data['Viaje']['vehiculo_id'])) {
            ...
            //$cercanos = $this->Vehiculo->Localizacion->getCercanos($data, 10, false);
            $cercanos = $data['Viaje']['vehiculo_id'];
...
//            ver esto creo está mal
    ...
    public function getBalanceUsuario($id){


 $qry = <<<SQL
select * from apptaxi_ivr_clientes_pagos where usuario_id = {$id}
SQL;

        $pagos = $this->query($qry);
        return $pagos;

    }


    public function getBalanceCliente($id){


 $qry = <<<SQL
select * from apptaxi_ivr_clientes_pagos where cliente_id = {$id}
SQL;
